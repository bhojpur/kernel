...
define pull_container
	docker pull bhojpur/$(1):$(shell jq '.["$(1)"]' containers/versions.json)
endef
...
define update_container_dependency
	...
	echo $(BASE_VERSION)
	cd containers/$(1) && perl -pi -e 's/FROM bhojpur\/(.*):.*/FROM bhojpur\/$$1:$(BASE_VERSION)/g' Dockerfile$(3)
endef
...
define build_container
	$(eval BASE_CONTAINER=$(shell cd containers/$(1) && cat Dockerfile$(3) | grep FROM | perl -p -e 's/FROM bhojpur\/(.*):.*/$$1/g'))
	echo $(BASE_CONTAINER)
	$(if $(findstring FROM,$(BASE_CONTAINER)),,$(call update_container_dependency,$(1),$(BASE_CONTAINER),$(3)))
	cd containers/$(1) && docker build -t bhojpur/$(2):build -f Dockerfile$(3) .
	$(eval CONTAINER_TAG=$(shell echo 'docker inspect bhojpur/$(2):build'))
	...
	docker tag bhojpur/$(2):build bhojpur/$(2):$(CONTAINER_TAG)
	$(call update_container_version,$(2),$(CONTAINER_TAG))
	@echo Built bhojpur/$(2):$(CONTAINER_TAG)
	docker rmi bhojpur/$(2):build
endef

define remove_container
	docker rmi -f bhojpur/$(1):$(shell cat containers/versions.json  | jq '.["$(1)"]')
endef
...
ifeq (,$(TARGET_OS))
	...
	docker build -t bhojpur/$@ -f Dockerfile .
	mkdir -p ./_build
	docker run --rm -v $(shell pwd)/_build:/opt/build:z -e TARGET_OS=$(TARGET_OS) bhojpur/$@
	#docker rmi -f bhojpur/$@
	@echo "Install finished! Bhojpur Kernel binary can be found at $(shell pwd)/_build/kernctl"
#----
